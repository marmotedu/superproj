...
```
projctl secret create SECRET_NAME
```
...
```
  ...
  projctl secret create foo
  
  # Create secret with a specified expire time and description
  projctl secret create foo --expires=1988121600 --description="secret for iam"
```

### Options

```
      --description string   The descriptin of the secret.
      --expires int          The expire time of the secret. (default 1667534344)
