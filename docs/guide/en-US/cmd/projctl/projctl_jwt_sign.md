...
```
projctl jwt sign SECRETID SECRETKEY
```
...
```
  ...
  projctl sign tgydj8d9EQSnFqKf iBdEdFNBLN1nR3fV
  
  # Sign a token with expires and sign method
  projctl sign tgydj8d9EQSnFqKf iBdEdFNBLN1nR3fV --timeout=2h --algorithm=HS256
```
...
```
      ...
      --audience string       Identifies the recipients that the JWT is intended for.
  ...
  -h, --help                  help for sign
      --issuer string         Identifies the principal that issued the JWT. (default "projctl")
